...
import time
import numpy as np
def print_current_Tfrs():
    nums, text = get_current_Tfrs()
...
def check():
    ...
    url = 'https://everydayastronaut.com/when-will-sn11-launch-live-updates/'
    page = requests.get(url)
    soup = BeautifulSoup(page.content, 'html.parser')
    result = soup.find('div', attrs={'class':'entry-content'})
    NET = result.text
    NET = NET.split('(NET) date')[1]
    NET = NET.split('Live Updates')[0]
    NET = NET.split('Flight')[1]
    NET = NET.split('\n')[0]
    print('current NET date: ' + NET)
    print('Last Checked at', time.ctime())
    del NET
    threading.Timer(3600, check).start()
from bs4 import BeautifulSoup
from win10toast import ToastNotifier
import threading
import time
